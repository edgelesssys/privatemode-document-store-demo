...
echo -e "${BOLD}Stopping and disabling systemd service...${RESET}"
if systemctl --user is-active --quiet "${APP_ID}.service" 2>/dev/null; then
    systemctl --user stop "${APP_ID}.service" >/dev/null 2>&1 || true
fi
if systemctl --user is-enabled --quiet "${APP_ID}.service" 2>/dev/null; then
    systemctl --user disable "${APP_ID}.service" >/dev/null 2>&1 || true
fi
...
# Detect stray service files elsewhere that could respawn the service
OTHER_SERVICES=()
for d in "$HOME/.config/systemd/user" /etc/systemd/user /etc/systemd/system; do
    [[ -d "$d" ]] || continue
    ...
    done < <(ls -1 "$d"/*.service 2>/dev/null || true)
done
if [[ ${#OTHER_SERVICES[@]} -gt 0 ]]; then
    echo -e "${YELLOW}Potential additional service files referencing ${APP_ID}:${RESET}"
    for f in "${OTHER_SERVICES[@]}"; do
        echo "  $f"
    done
